...
def draw_hex(t, s):
    ...
    init_x = t.x
    init_y = t.y
    init_heading = t.heading

    for triangle in range(6):
        t.left(60)
        draw_triangle(t, s, 'orange')

    t.move(init_x, init_y)
    t.heading = init_heading
    t.flush()
...
def draw_spiral_helper(t, side, ang, n, sp):
    ...
    init_color = t.color
    init_speed = t.speed
    init_visible = t.visible
    init_draw_mode = t.drawmode
    colors = ['blue', 'red', 'orange']*((n//3)+1)
    for edge in range(n):
        t.color = colors[edge]
        t.forward((edge+1)*side)
        t.left(ang)
    ...
    t.color = init_color
    t.speed= init_speed
    t.visible = init_visible
    t.drawmode = init_draw_mode
...
def multi_polygons(w, side, k, n, sp):
    ...
    assert is_valid_length(side), report_error('side is not a valid length',side)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert is_valid_iteration(k), report_error('Invalid number of polygons', k)
    assert type(n) == int, repr(n) + 'is not an integer'
    assert n>=3, repr(n) + 'is an invalid number of sides'
    
    # HINT: w.clear() clears window.
    # HINT: Set the visible attribute to False at the end, and remember to flush
    w.clear()
    t = Turtle(screen= w, heading= 270)
...
def multi_polygons_helper(t, side, k, n, sp):
    ...
    init_speed = t.speed
    init_heading = t.heading
    init_drawmode = t.drawmode
    colors = ['green', 'blue']*((k//2) + 1)
    for polygon in range(k):
        t.color = colors[polygon]
...
def vicsek(w, size, d, sp):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(size), report_error('Invalid size', size)
    assert is_valid_speed(sp), report_error('Invalid pen speed', sp)
...
def vicsek_helper(p, x, y, size, d):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_valid_penmode(p), report_error('Invalid pen mode', p)
    assert is_number(x), report_error('Invalid x-coordinate', x)
    assert is_number(y), report_error('Invalid y-coordinate', y)
    assert is_valid_length(size), report_error('size is an invalid size', size)
    assert is_valid_depth(d), report_error('d is not a valid depth', d)
    ...
    #recursive cases
    up = vicsek_helper(p, x, y + size/3, size/3, d-1)
    down = vicsek_helper(p, x, y - size/3, size/3, d-1)
    left = vicsek_helper(p, x - size/3, y , size/3, d-1)
    right = vicsek_helper(p, x + size/3, y , size/3, d-1)
    center = vicsek_helper(p, x , y , size/3, d-1)
...
def branches(w, hght, ang, d, sp):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(hght), report_error('Invalid height', hght)
    assert is_number(ang), report_error('Invalid angle', ang)
    assert ((ang> 0) and (ang < 180)), report_error('Invalid angle', ang)
    assert is_valid_depth(d), report_error('Invalid recursion depth', d)
    assert is_valid_speed(sp), report_error('Invalid speed', sp)
    
    # HINT: w.clear() clears window.
    # HINT: Set the visible attribute to False at the end, and remember to flush
    w.clear()

    t = Turtle(screen=w, color='forest green', position=(0 , -hght/2), heading = 90)
